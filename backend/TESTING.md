...
| §5.1 operator fan-out | an operator's subscription set is fixed (never per-conversation); an assigned support conversation reaches the agents channel; an unassigned one never does, since that channel reaches every operator | `realtime/parity_test.go: TestChannelSetDoesNotGrowWithTheQueue`, `domain/agent_fanout_test.go` |
