...
func TestEmailChannelGetAndUpdate(t *testing.T) {
	...
	w := h.Request("GET", "/v1/channels/email").Cookie("sild_admin", owner).Do()
...
func TestEmailChannelRBAC(t *testing.T) {
	...
	if w := h.Request("GET", "/v1/channels/email").Cookie("sild_admin", agent).Do(); w.Code != http.StatusForbidden {
		t.Fatalf("agent must not read channels, got %d %s", w.Code, w.Body)
	}
	if w := h.Request("GET", "/v1/channels/email").Do(); w.Code != http.StatusUnauthorized {
