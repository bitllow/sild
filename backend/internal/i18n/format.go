...
// renderAccessors writes a constant per key for a tenant's own build to look its
// strings up through: renaming a key in the editor then removes its member, and the
// stale lookup fails the build instead of reaching a customer as a blank.
//
...
func renderAccessors(format Format, rows []Row) ([]byte, error) {
	...
	case FormatSwift:
		return accessorFile(swiftAccessors, plain, plural), nil
	case FormatTypeScript:
...
func accessorFile(emit func(plain, plural []string) []string, plain, plural []string) []byte {
	lines := append([]string{
...
func kotlinAccessors(plain, plural []string) []string {
	out := []string{"object SildProjectKeys {"}
	for _, key := range plain {
		out = append(out, fmt.Sprintf("    const val %s: String = %q", ident(key), key))
	}
	out = append(out, "", "    /** Keys addressed by a count. */", "    object Plural {")
	for _, base := range plural {
		out = append(out, fmt.Sprintf("        const val %s: String = %q", ident(base), base))
	}
	return append(out, "    }", "}")
}

func swiftAccessors(plain, plural []string) []string {
	out := []string{"public enum SildProjectKeys {"}
	for _, key := range plain {
		out = append(out, fmt.Sprintf("    public static let %s = %q", ident(key), key))
	}
	out = append(out, "", "    /// Keys addressed by a count.", "    public enum Plural {")
	for _, base := range plural {
		out = append(out, fmt.Sprintf("        public static let %s = %q", ident(base), base))
	}
	return append(out, "    }", "}")
}
