...
// with no backend — these run under a plain `./gradlew test`.
class SildClientCallbackTest {
    ...
    @Test fun sendWithNoActiveThreadReportsFailure() = runBlocking {
    ...
    @Test fun emptySendReportsFailure() = runBlocking {
        val client = failingClient()
    ...
    @Test fun failedSendReportsFailureAndAppendsNothing() = runBlocking {
        val client = failingClient()
        client.openAndAwaitActive("c1")
        val result = CompletableDeferred<Boolean>()
    ...
    @Test fun failedSupportRequestReportsNull() = runBlocking {
        val client = failingClient()
