...
object BrandTheme {
    ...
    fun shade(hex: String, pct: Double = 0.85): String {
        ...
        // floor(x + 0.5), like the web widget's Math.round — not Kotlin's round(),
        // which breaks ties to even and would darken 0x0A to 8 instead of 9.
        fun c(v: Int) = floor(v * pct + 0.5).toInt().coerceIn(0, 255)
        return "#" + listOf(c(r), c(g), c(b)).joinToString("") {
